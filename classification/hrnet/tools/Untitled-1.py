...
def main(save_):
    ...
    train_dataset_ =  datasets.ImageFolder(traindir, transforms.ToTensor())   
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset_,batch_size=config.TRAIN.BATCH_SIZE_PER_GPU*len(gpus),shuffle=True)
    mean_, std = com_md.get_mean_std(train_loader)

    normalize = transforms.Normalize(mean=mean_,
                                     std=std)
...
if __name__ == '__main__':
    ...
    #     fiv5toone.imgs_5_1(i, k)
    #     main('非序列图像分类' +':' +str(k)+str('折') + str(i) + '\n')
    for i in range(1, k+1):
        fiv5toone.case_5_1(i, k)
        main('CASE class' +':' +str(k)+str('折') + str(i) + '\n')
